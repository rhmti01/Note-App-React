...
function ModalOnEdit({ note, setModal, recordEditNote }) {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-md flex items-center justify-center">

            {/* space out of div */}
            <div
                onClick={() => setModal(false)}
                class="  cursor-pointer  inset-0 absolute z-20 w-full h-screen   "></div>
            ...
            <div className=" xl:scale-100 xx:scale-[0.87] mm:scale-[0.85] rr:scale-[0.82] ss:scale-[0.80] bg-white dark:bg-gray-900 p-7 rounded-2xl w-[350px] md:w-[420px] z-50 dark:shadow-indigo-900 ">
                <h2 className="text-gray-900 dark:text-gray-100 text-center text-2xl font-bold">Edit Todo</h2>
                <div className="mt-5">
                    <label className=" text-gray-800 dark:text-gray-200 block font-semibold text-[17px] ">Title</label>
                    <input
                        type="text"
                        className="w-full  py-2.5 px-3 dark:bg-gray-900 border border-stone-300 dark:border-gray-800 dark:text-white mx-auto  mt-3  rounded-[10px] outline-none focus:shadow-lg  focus:border-indigo-600   "
                        value={title}
